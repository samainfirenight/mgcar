...
export async function getWorldEntry(name, data, entry) {
	...
	// MGB Edit char Two char two
	const characterFilterLabelTwo = template.find('label[for="characterFilterTwo"] > small');
    characterFilterLabelTwo.text(entry.characterFilterTwo?.isExclude ? 'Exclude Character(s) Two' : 'Filter to Character(s) Two');
	// end edit
...
// MGB Edit Char two exclude checkbox Two

    const characterExclusionInputTwo = template.find('input[name="character_exclusionTwo"]');
    characterExclusionInputTwo.data('uid', entry.uid);
    characterExclusionInputTwo.on('input', async function () {
        const uid = $(this).data('uid');
        const value = $(this).prop('checked');
        characterFilterLabelTwo.text(value ? 'Exclude Character(s) Two' : 'Filter to Character(s) Two');
        if (data.entries[uid].characterFilterTwo) {
            if (!value && data.entries[uid].characterFilterTwo.names.length === 0 && data.entries[uid].characterFilterTwo.tags.length === 0) {
                delete data.entries[uid].characterFilterTwo;
            } else {
                data.entries[uid].characterFilterTwo.isExclude = value;
            }
        } else if (value) {
            Object.assign(
                data.entries[uid],
        ...
        // Verify names to exist in the system
        if (data.entries[uid]?.characterFilterTwo?.names?.length > 0) {
            for (const name of [...data.entries[uid].characterFilterTwo.names]) {
                if (!getContext().characters.find(x => x.avatar.replace(/\.[^/.]+$/, '') === name)) {
                    console.warn(`World Info: Character ${name} not found. Removing from the entry filter.`, entry);
                    data.entries[uid].characterFilterTwo.names = data.entries[uid].characterFilterTwo.names.filter(x => x !== name);
                }
            }
        }

        setWIOriginalDataValue(data, uid, 'character_filterTwo', data.entries[uid].characterFilterTwo);
        await saveWorldInfo(name, data);
    });	
...
// end EDIT
	...
	// MGB Edit char Two
	characterExclusionInputTwo.prop('checked', entry.characterFilterTwo?.isExclude ?? false).trigger('input');
    const characterFilterTwo = template.find('select[name="characterFilterTwo"]');
    characterFilterTwo.data('uid', entry.uid);
	// end edit
	...
	// MGB Edit char Two
    const charactersTwo = getContext().characters;
    charactersTwo.forEach((character) => {
        const option = document.createElement('option');
        const name = character.avatar.replace(/\.[^/.]+$/, '') ?? character.name;
        option.innerText = name;
        option.selected = entry.characterFilterTwo?.names?.includes(name);
        option.setAttribute('data-type', 'character');
        characterFilterTwo.append(option);
    });
	// end edit	
	...
	// MGB Edit char Two
	const tagsTwo = getContext().tags;
    tagsTwo.forEach((tag) => {
        const option = document.createElement('option');
        option.innerText = `[Tag] ${tag.name}`;
        option.selected = entry.characterFilterTwo?.tags?.includes(tag.id);
        option.value = tag.id;
        option.setAttribute('data-type', 'tag');
        characterFilterTwo.append(option);
    });
...
// MGB Edit char Two	
    characterFilterTwo.on('mousedown change', async function (e) {
        // If there's no world names, don't do anything
        if (world_names.length === 0) {
            e.preventDefault();
            return;
        ...
        const uid = $(this).data('uid');
        const selected = $(this).find(':selected');
        if ((!selected || selected?.length === 0) && !data.entries[uid].characterFilterTwo?.isExclude) {
            delete data.entries[uid].characterFilterTwo;
        } else {
            const names = selected.filter('[data-type="character"]').map((_, e) => e instanceof HTMLOptionElement && e.innerText).toArray();
            const tags = selected.filter('[data-type="tag"]').map((_, e) => e instanceof HTMLOptionElement && e.value).toArray();
            Object.assign(
                ...
                {
                    characterFilterTwo: {
                        isExclude: data.entries[uid].characterFilterTwo?.isExclude ?? false,
                        names: names,
                        tags: tags,
                    },
                },
...
export async function checkWorldInfo(chat, maxContext, isDryRun) {
    ...
    while (scanState) {
        ...
        for (let entry of sortedEntries) {
			...
			//	const getCharaFilename() = getCharaFilename();

                if (filtered) {
                    console.debug(`-------- MGB filtered out by character. -------- ` + getCharaFilename());
                    continue;
                }
            }
// Start MGB Tag Filters here

// MGB Check For Tag Filters here

            if (entry.characterFilter && entry.characterFilter?.tags?.length > 0) {
                const tagKey = getTagKeyForEntity(this_chid);
				const tagKeyGroup = getTagKeyForEntity(selected_group) 

                if (tagKey || tagKeyGroup) {
                    const tagMapEntry = context.tagMap[tagKey];
					const tagMapEntryGroup = context.tagMap[tagKeyGroup];
					console.debug(` -------- MGB Tag Key MGB -------- ` + tagMapEntry);
					console.debug(` -------- MGB Tag Group MGB -------- ` + tagMapEntryGroup);
                    if (Array.isArray(tagMapEntry)) {
                        ...
                        if (filtered) {
							console.debug(`-------- MGB filtered out by tag by character. -------- `  + tagKey + ` ` + tagKeyGroup + ` ` + getCharaFilename());
                            console.debug(`WI entry ${entry.uid} filtered out by tag by character`);
                        //    continue;
// MGB EDIT XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
					
							if (Array.isArray(tagMapEntry)) {
								const includesTag = tagMapEntry.some((tag) => entry.characterFilterTwo?.tags.includes(tag));
								const filtered = entry.characterFilterTwo?.isExclude ? includesTag : includesTag;
								
								if (filtered) {
									console.debug(`-------- MGB filtered out by tag by character two. -------- `  + tagKey + ` ` + tagKeyGroup + ` ` + getCharaFilename());
									console.debug(`WI entry ${entry.uid} filtered out by tag by character two`);
									continue;
								}else{
										if (Array.isArray(tagMapEntryGroup)) {
											// If tag map intersects with the tag exclusion list, skip
											const includesTag = tagMapEntryGroup.some((tag) => entry.characterFilter.tags.includes(tag));
											const filtered = entry.characterFilter.isExclude ? includesTag : !includesTag;

											if (filtered) {
												console.debug(`-------- MGB filtered out by tag by group. -------- `  + tagKey + ` ` + tagKeyGroup + ` ` + getCharaFilename());
												console.debug(`WI entry ${entry.uid} filtered out by tag by group`);
												continue;
											}
										}
									// continue;
