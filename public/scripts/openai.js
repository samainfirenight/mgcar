...
async function sendOpenAIRequest(type, messages, signal) {
	...
	// MGB Edit
    const stream = oai_settings.stream_openai && !isQuiet && !isScale && !(isGoogle && oai_settings.google_model.includes('bison')) && !(isOAI && oai_settings.openai_model.startsWith('o1-')) && !(isOAI && oai_settings.openai_model.startsWith('o3-'));
...
// MGB Edit
    if (isOAI && oai_settings.openai_model.startsWith('o3-')) {
        generate_data.messages.forEach((msg) => {
            if (msg.role === 'system') {
                msg.role = 'user';
            }
